...
for(const lang of ['en','es'])for(const file of ['index.html','aviso-legal.html','privacidad.html']){
 ...
 if(file==='index.html'){
  const titles=lang==='es'?['Nuestro trabajo','Bitácora','Así trabajamos','Conócenos','Hablemos']:['Our work','Field log','How we work','Meet the team','Let’s talk'];
  ...
  assert(footer.includes(`href="${lang==='es'?'/es/aviso-legal.html':'/en/aviso-legal.html'}"`),'Legal notice remains in footer and preserves language');
  ...
  assert(html.includes(lang==='es'?'Siempre en inglés.':'Always in English.'));
